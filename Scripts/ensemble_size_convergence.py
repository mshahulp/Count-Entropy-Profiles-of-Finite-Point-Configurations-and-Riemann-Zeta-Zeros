"""
Ensemble-size convergence study for the count-entropy manuscript.

Purpose
-------
Determine a defensible ensemble size R for Poisson and GUE reference ensembles
at a fixed finite-configuration design. The script:

1. generates a master ensemble of count-entropy profiles;
2. uses a reproducible random seed;
3. compares candidate ensemble sizes R against the master ensemble;
4. quantifies convergence of:
   - the ensemble mean profile,
   - the 2.5% and 97.5% pointwise realization quantiles,
   - the Monte Carlo standard error (MCSE) of the mean;
5. writes all numerical results, configuration metadata, and figures to disk.

The count law p_L(k) is evaluated exactly with respect to the sliding-window
origin measure (up to floating-point arithmetic); no Monte Carlo sampling of
window origins is used.

GUE generation
--------------
The code uses the beta-Hermite tridiagonal model at beta=2. In the convention
implemented below, the tridiagonal entries are

    diagonal ~ N(0, 1)
    off-diagonal_i ~ chi_{2(N-i)} / sqrt(2),   i=1,...,N-1

and the eigenvalues are divided by sqrt(N), giving the macroscopic
semicircle scale [-2,2]. Semicircle-CDF unfolding is then applied.

The central retained fraction is controlled by parent_factor. With the default
parent_factor=2, N=2n and the central n unfolded eigenvalues are retained.

IMPORTANT
---------
The default "full" design can be computationally expensive. Start with
--quick to verify the environment and pipeline. Then run the full design on
the machine that will produce the archived manuscript results.

Example
-------
python ensemble_size_convergence.py --quick
python ensemble_size_convergence.py
"""
...
def exact_count_entropy(points: np.ndarray, a: float, b: float, L: float) -> float:
    """
    Exact sliding-window count entropy for a finite point configuration.

    Window origins t are uniform on I_L=[a,b-L], and
        N(t;L) = # {x_i in [t,t+L)}.

    Each point x_i contributes for origins t in (x_i-L, x_i], intersected
    with I_L. Endpoints are measure-zero and do not affect the law.

    The algorithm sweeps all clipped interval endpoints and accumulates the
    exact Lebesgue length associated with each count value.
    """
    W = b - a
    if not (0.0 < L < W):
    ...
    # Events: entering a contribution interval increases count; leaving decreases.
...
def poisson_configuration(n: int, rng: np.random.Generator):
    """
    Fixed-cardinality Poisson control:
    n iid Uniform(0,n) points, sorted, with I=[0,n].

    This is the binomial point process, equivalently a homogeneous Poisson
    process on [0,n] conditioned on exactly n points.
    """
...
def gue_configuration(n: int, parent_factor: float, rng: np.random.Generator):
    """
    Generate a beta=2 Hermite tridiagonal realization, unfold by semicircle CDF,
    retain central n points, and define observation boundaries using the nearest
    discarded neighbours. Finally apply one affine scaling so that W=b-a=n.
...
def convergence_from_master(
    ...
    """
    Candidate-R convergence assessed by repeated subsampling from a master ensemble.

    Reference:
      mean and pointwise 2.5/97.5% quantiles of all master realizations.

    For each R, repeated subsets of R distinct realizations are drawn without
    replacement. Metrics compare subset estimates with master estimates.
    """
...
def prefix_doubling_diagnostic(profiles: np.ndarray, candidate_r, model: str) -> pd.DataFrame:
    """
    Secondary transparent diagnostic:
    compare nested prefix means at R and 2R.

    This is not the primary selection method because it depends on ordering,
    but it is useful as an auditable cross-check.
    """
    rows = []
    Rmax = profiles.shape[0]
    for R in candidate_r:
