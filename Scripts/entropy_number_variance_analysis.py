"""
Entropy-number-variance analysis for the count-entropy manuscript.

Purpose
-------
This stage empirically examines the relationship between Shannon count entropy
H(L) and number variance Sigma^2(L), using the already validated zeta count-law
outputs. It is designed to complement the theoretical Section 3 result

    H <= 1/2 log(2*pi*e*(Sigma^2 + 1/12))

(Massey's integer-valued entropy bound), and to test empirically whether nearly
equal variances can correspond to materially different count entropies.

Inputs
------
1. zeta_count_entropy_profiles.csv
2. zeta_count_pmf_sparse.csv

The sparse PMF file is required because candidate near-equal-variance /
different-entropy examples must be verified from the underlying count laws,
rather than inferred from H and variance alone.

Analyses
--------
A. Global and within-regime H-versus-variance association:
   Pearson and Spearman correlations.

B. Massey-bound audit:
   bound value, slack = bound-H, minimum slack, any numerical violations.

C. Near-equal-variance / different-entropy search:
   searches pairs of observations from DIFFERENT source configurations.
   To avoid trivial comparisons at different scales, the primary search requires
   the SAME L value. Candidate pairs are ranked by entropy difference while
   controlling variance closeness.

   Thresholds are reported at several relative variance tolerances:
       0.1%, 0.25%, 0.5%, 1.0%, 2.0%
   with a small absolute floor to handle low variances.

D. PMF verification for the strongest examples:
   reconstructs the sparse count laws and computes total variation distance,
   means, variances, entropies, and support sizes.

E. Optional surrogate archive analysis:
   if surrogate_profiles_full.npz is supplied, reports H-variance correlations
   across the shuffled ensemble as a supplementary robustness analysis.

This script is descriptive. Correlations are not used to claim functional
equivalence or statistical independence.
"""
...
def main():
    ...
    # PMF verification. Deduplicate candidate pair identities, preferring
    # stringent tolerance and large entropy separation.
