"""
Quantitative comparison of validated Riemann-zeta count-entropy profiles with
the archived GUE master ensemble.

IMPORTANT
---------
The ensemble-size convergence study used R_master=500 to assess candidate R and
the study subsequently froze R=400 for the principal GUE reference. Therefore,
this comparison uses the FIRST 400 archived GUE realizations by default. It does
not silently use all 500 master realizations.

Inputs
------
1. zeta_count_entropy_profiles.csv
2. gue_master_profiles.csv (or gue_master_profiles(1).csv)

Metrics
-------
For profile f on the common 200-point L grid and reference GUE mean mu:

    RMSE(f,mu) = sqrt(mean_L (f(L)-mu(L))^2)
    SUP(f,mu)  = max_L |f(L)-mu(L)|

For each of the selected GUE realizations the same distances to the GUE ensemble
mean are computed. Two reference versions are archived:

A. common-mean distances:
   every GUE realization is compared with the same R-realization ensemble mean.
   These are directly analogous to the zeta-to-GUE distances.

B. leave-one-out distances:
   realization r is compared with the mean of the other R-1 realizations.
   This removes the small self-inclusion effect and is used for the primary
   empirical tail comparison.

For a zeta distance d, the empirical upper-tail fraction is

    (1 + #{GUE distances >= d}) / (R + 1)

using the leave-one-out GUE distance distribution. This is a descriptive
reference-tail measure, not a formal hypothesis-test p-value.

Outputs
-------
zeta_gue_comparison_results/
  gue_selected_profile_summary.csv
  gue_self_distance_distribution.csv
  zeta_gue_block_distances.csv
  zeta_gue_regime_distance_summary.csv
  zeta_gue_pointwise_band_coverage.csv
  zeta_gue_comparison_manifest.json
"""
