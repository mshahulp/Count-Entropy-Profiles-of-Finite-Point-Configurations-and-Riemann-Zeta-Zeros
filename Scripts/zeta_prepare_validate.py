"""
Prepare and validate Riemann-zeta zero blocks for the count-entropy study.
...
Datasets used
-------------
zeros1 : first 100,000 zeros; first 10,000 used as the low-height parent block.
zeros3 : zeros 10^12+1 ... 10^12+10^4.
zeros4 : zeros 10^21+1 ... 10^21+10^4.
zeros5 : zeros 10^22+1 ... 10^22+10^4.

For each 10,000-zero parent sequence, four non-overlapping blocks of
n=2000 retained zeros are formed. Each block uses one immediate guard zero
on each side. The retained starts (0-based indices into the parent sequence)
are 1, 2001, 4001, 6001.

Unfolding
---------
For each ordinate gamma, use the smooth Riemann-von Mangoldt term

    Nbar(T) = T/(2*pi)*log(T/(2*pi)) - T/(2*pi) + 7/8.

Only differences of Nbar matter for the subsequent affine normalization.
The oscillatory term S(T) is NOT included.

For high-height Odlyzko files, the tabulated values are offsets from a large
integer base stated in the file header. The code parses the offset base and
reconstructs gamma with arbitrary-precision mpmath arithmetic. This avoids
catastrophic loss of local spacing information at heights 10^21-10^22.

Boundary normalization
----------------------
If x_0 and x_{n+1} are unfolded guard points and x_1,...,x_n are retained,

    a = (x_0 + x_1)/2
    b = (x_n + x_{n+1})/2

and a single affine map sends I=[a,b] to [0,n]:

    y_j = n * (x_j-a)/(b-a).

The resulting configuration therefore has W=n and exact mean intensity n/W=1.

This script DOES NOT compute count entropy. It only downloads, parses,
constructs, unfolds, normalizes, validates, and exports the zeta blocks.
"""
...
def parse_odlyzko_table(text: str) -> tuple[list[mp.mpf], mp.mpf, dict]:
    """
    Parse an Odlyzko table.

    zeros1 contains full ordinates, one numeric line each.
    zeros3/4/5 contain offsets from an integer base stated in the header.

    Returns
    -------
    values : full gamma ordinates as mp.mpf
    base   : parsed offset base (0 for full-ordinate table)
    info   : parser metadata
    """
...
def unfold_locally(gammas: list[mp.mpf]) -> list[mp.mpf]:
    """
    Compute local unfolded coordinates with high precision.

    Subtract Nbar(gamma[0]) immediately so that we never carry huge unfolded
    coordinates farther than necessary. This subtraction is done in mp arithmetic.
    """
...
def make_block(
    ...
    """
    start is the 0-based index of the first retained zero.
    Requires guard points at start-1 and start+n.
    """
    if start < 1:
    ...
    # is safe for the subsequent count-entropy computation.
